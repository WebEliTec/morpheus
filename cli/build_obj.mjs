...
class MorphSrcBuildDirectoryBuilder {
    ...
    async createSingleResourceFiles( nodeId ) {
      ...
      this.processModuleRegistry( moduleRegistry, nodeId );
    ...
    processModuleRegistry( moduleRegistry, nodeId ) {
      ...
      Object.entries( moduleRegistry ).forEach( ( [ moduleId, moduleRegistryItem ] ) => {
        console.log( 'moudleId '+ moduleId );
      })

      /*
      const moduleItems = Object.entries ( moduleRegistry );

      moduleItems.forEach( ( [ moduleId, moduleRegistryItem ] ) => {
